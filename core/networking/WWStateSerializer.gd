...
static func _build_card(state: WWPlayerState, viewer: WWPlayerState) -> Dictionary:
	...
	return {
		...
		"avatar_data": state.player.avatar_data if "avatar_data" in state.player else {},
		"alive": state.alive,
		"role": state.role.id if (show_role and state.role) else "",
		"role_display": state.role.display_name if (show_role and state.role) else "",
		"role_icon": state.role.icon if (show_role and state.role) else "",
